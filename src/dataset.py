        ...
        if self.drop_temp_s1 + self.drop_temp_s2 > 0:
            ...
            for sat in self.sats:
                if "S1" in sat:
                    ...
                    selected_dates[sat] = (
                            np.random.rand(data[sat].shape[0]) > self.drop_temp_s1
                    )
                elif "S2" in sat:
                    selected_dates[sat] = (
                            np.random.rand(data["S2"].shape[0]) > self.drop_temp_s2
                    )
            data = {sat: d[selected_dates[sat]] for sat, d in data.items()}
            dates = {s: d[selected_dates[s]] for s, d in dates.items()}
