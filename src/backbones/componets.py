...
import torch
from SwinUnetV2.network.swin_unet_v2 import Mlp_Relu, PatchExpand, Mlp
...
class SwinTransformerShareBlock(nn.Module):
...
class ShareWindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None, attn_map=None):
        """
        Args:
...
class ShareBasicLayer_up(nn.Module):
    """ A basic Swin Transformer layer for one stage.
    Args:
        dim (int): Number of input channels.
        input_resolution (tuple[int]): Input resolution.
