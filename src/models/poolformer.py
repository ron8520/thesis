...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 20, 'input_size': (10, 128, 128), 'pool_size': None,
...
class PoolFormer(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None, 
        ...
        ## Sentinel-2 in_chans is 10, other is 3
        self.patch_embed = PatchEmbed(
            patch_size=in_patch_size, stride=in_stride, padding=in_pad,
            in_chans=10, embed_dim=embed_dims[0])
...
class Feature_aliasing(torch.nn.Module):
...
class Feature_reduce(torch.nn.Module):
...
class Prediction_head(torch.nn.Module):
